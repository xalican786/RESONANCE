// src/adapters/modempay.js — RESONANCE ModemPay bridge
// Universal adapter — same interface across all SSS systems
// USDC profit → fiat via shadow-routed treasury withdrawal
...
export async function send(key, params) {
  ...
  const net = network || (
    ...
    : type?.includes('intl') ? 'international'
    : 'crypto'
  )

  const fee    = amount * (FEES[net] || 0.015)
  const netAmt = amount - fee
  const ref    = `RESONANCE | JUPITERR | ${Date.now()}`
