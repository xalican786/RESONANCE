// src/chains.js — RESONANCE chain monitor (Worker)
// 20 chains — every swap, price event, oracle update
// Everything is a potential resonance trigger
// Memory cap: 80MB (resourceLimits in index.js)
// 500ms stagger — prevents burst on boot
  ...
  const connect = () => {
    ...
    try {
      socket = new ws(chain.ws, {
        ...
        maxPayload:        32 * 1024,   // 32KB — tight, prevents OOM
    ...
    socket.on('open', () => {
      ...
      // Subscribe to Uniswap V3 swaps — highest volume signal
    ...
    socket.on('message', raw => {
      try {
        ...
        parentPort.postMessage({
          type:  'swap',
          chain: chain.name,
          hash:  msg.params.result.transactionHash.slice(0,20),
          block: msg.params.result.blockNumber,
        })
      } catch {}
    })
...
console.log(`[CHAINS] ${WS_CHAINS.length} chains connecting | 500ms stagger | every event = resonance trigger`)
